...
IBGE population data published through DATASUS. These files use `.dbf`
format (not `.dbc`).

``` r

# Census and estimates
df <- datasus_fetch(source = "IBGE", file_type = "POP", year = 2012)

# Estimates by sex and age
df <- datasus_fetch(source = "IBGE", file_type = "POPS", year = 2012)

# TCU estimates
df <- datasus_fetch(source = "IBGE", file_type = "POPT", year = 2021)
```
